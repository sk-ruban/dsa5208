...
def prepare_data():
    ...
    # Convert the y data to float to match x post standardized
    return (X_train_scaled, X_test_scaled, 
            y_train.values.astype(np.float64), 
            y_test.values.astype(np.float64))
...
def kernel_ridge_regression(X_train, y_train, X_test, y_test, sigma, lambda_reg, comm):
    ...
    if rank == 0:
        ...
        y_pred_train = np.clip(K @ alpha, 0, 500001)  # Clip predictions to [0, 500000]
        y_pred_test = np.clip(K_test @ alpha, 0, 500001)  # Clip predictions to [0, 500000]
